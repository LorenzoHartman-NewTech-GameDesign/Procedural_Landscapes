...
public class FractalTree : MonoBehaviour {
    ...
    public float branchRotation = 30f;
    ...
    void Start() {
        ...
        treeObject = GameObject.Find("2dTree");
        treeTransform = treeObject.transform;
        GenerateTree();
        ...
        foreach (char c in p_input)
        {
            switch (c)
            {
                case 'F': // Draw line of length lastBranchLength, in direction of lastAngle
                    ...
                    break;
                case '+': // Rotate +30
                    lastPoint.Angle.x += branchRotation;
                    break;
                case '[': // Save State
                    returnValues.Push(lastPoint);
                    break;
    ...
    void Update ()
    {
        ...
        if (resetPivot == true)
        {
            ...
            treeTransform.position = new Vector3(treeTransform.position.x, (treeTransform.position.y + 5f), treeTransform.position.z);
            Debug.Log("changed");
        }
