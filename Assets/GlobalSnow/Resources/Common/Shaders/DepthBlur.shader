...
SubShader {
	...
	v2fCross vertBlurH(appdata v) {
		...
		float4 inc  = half4(_MainTex_TexelSize.x * 1.3846153846, 0, 0, 0);	
    	o.uv1 = float4(v.texcoord - inc, 0, 0);	
    	o.uv2 = float4(v.texcoord + inc, 0, 0);	
		float4 inc2 = half4(_MainTex_TexelSize.x * 3.2307692308, 0, 0, 0);	
	...
	v2fCross vertBlurV(appdata v) {
    	...
    	#endif 
    	o.uv = float4(v.texcoord, 0, 0);
    	float4 inc  = half4(0, _MainTex_TexelSize.y * 1.3846153846, 0, 0);	
    	o.uv1 = float4(v.texcoord - inc, 0, 0);
    	o.uv2 = float4(v.texcoord + inc, 0, 0);	
    	float4 inc2 = half4(0, _MainTex_TexelSize.y * 3.2307692308, 0, 0);	
    	o.uv3 = float4(v.texcoord - inc2, 0, 0);	
    	o.uv4 = float4(v.texcoord + inc2, 0, 0);
    	return o;
	}

    #define DDEPTH(uv) tex2Dlod(_MainTex,uv)
    float4 fragBlur (v2fCross i): SV_Target {
        if (i.uv.x<=0.01 || i.uv.x>=0.99 || i.uv.y<=0.01 || i.uv.y>=0.99) return float4(2.0, 0.0, 0.0, 0.0);

        float4 pixel = DDEPTH(i.uv) * 0.2270270270
